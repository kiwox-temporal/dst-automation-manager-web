...
import net.kiwox.manager.dst.appium.service.interfaces.IInternationalWebTestService;
...
@Service
public class InternationalWebTestServiceImpl implements IInternationalWebTestService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(InternationalWebTestServiceImpl.class);
	...
	public void runTest(ControllerProbe probe, TestResult result) {
		...
		addUrl(script, parameterService.getString(Parameter.INTERNATIONAL_WEB_1));
		addUrl(script, parameterService.getString(Parameter.INTERNATIONAL_WEB_2));
		addUrl(script, parameterService.getString(Parameter.INTERNATIONAL_WEB_3));
		...
		try {
			...
			codeMessageService.logInfo("DST-INTERNATWEB-001", probe.getId(), script);
			LOGGER.error("[DST-INTERNATWEB-001] Error starting process", e);
		...
		if (json == null) {
			codeMessageService.logInfo("DST-INTERNATWEB-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		String code = json.optString("code", "").replace("WEB", "INTERNATWEB");
		String message = json.optString("message");
		if (json.optBoolean("error")) {
			if ("DST-INTERNATWEB-003".equals(code)) {
		...
		try {
			...
			LOGGER.error("[DST-INTERNATWEB-002] Error getting time from json", e);
			codeMessageService.logInfo("DST-INTERNATWEB-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		LOGGER.info("International web test executed successfully and stored in [{}]", result);
	}
