...
import net.kiwox.manager.dst.appium.service.interfaces.IDownloadTestService;
...
@Service
public class DownloadTestServiceImpl implements IDownloadTestService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(DownloadTestServiceImpl.class);
	...
	public void runTest(ControllerProbe probe, TestResult result) {
		...
		script.append(" -url \"");
		script.append(parameterService.getString(Parameter.DOWNLOAD_MOBILE_URL));
		script.append("\" -file \"");
		script.append(parameterService.getString(Parameter.DOWNLOAD_PATH_MOBILE_FILE));
		script.append("\"");
		...
		try {
			...
			codeMessageService.logInfo("DST-DOWNLOAD-001", probe.getId(), script);
			LOGGER.error("[DST-DOWNLOAD-001] Error starting process", e);
		...
		if (json == null) {
			codeMessageService.logInfo("DST-DOWNLOAD-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		if (error && StringUtils.isNotEmpty(code)) {
			if ("DST-DOWNLOAD-003".equals(code)) {
		...
		try {
			...
			LOGGER.error("[DST-DOWNLOAD-002] Error getting speed from json", e);
			codeMessageService.logInfo("DST-DOWNLOAD-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		LOGGER.info("Download test executed successfully and stored in [{}]", result);
	}
}
