...
import net.kiwox.manager.dst.appium.service.interfaces.IWhatsappPhotoTestService;
...
@Service
public class WhatsappPhotoTestServiceImpl implements IWhatsappPhotoTestService{
	
	private static final Logger LOGGER = LoggerFactory.getLogger(WhatsappPhotoTestServiceImpl.class);
	...
	public void runTest(ControllerProbe probe, TestResult result) {
		...
		String folder = parameterService.getString(Parameter.WHATSAPP_FOLDER_NAME);
		...
		try {
			...
			codeMessageService.logInfo("DST-WAPHOTO-001", probe.getId(), script);
		...
		if (json == null || !json.has("time")) {
			codeMessageService.logInfo("DST-WAPHOTO-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		try {
			...
			LOGGER.error("[DST-WAPHOTO-002] Error getting time from json", e);
			codeMessageService.logInfo("DST-WAPHOTO-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		LOGGER.info("Whatsapp photo test executed successfully and stored in [{}]", result);
	}
