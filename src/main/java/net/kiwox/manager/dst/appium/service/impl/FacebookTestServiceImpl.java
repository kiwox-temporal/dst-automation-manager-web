...
import net.kiwox.manager.dst.appium.service.interfaces.IFacebookTestService;
...
@Service
public class FacebookTestServiceImpl implements IFacebookTestService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(FacebookTestServiceImpl.class);
	...
	public void runTest(ControllerProbe probe, TestResult result) {
		...
		script.append(" -user \"");
		script.append(parameterService.getString(Parameter.FACEBOOK_USERNAME));
		script.append("\" -pass \"");
		script.append(parameterService.getString(Parameter.FACEBOOK_PASSWORD));
		script.append("\"");
		
		Test test = result.getTest();
		
		String folderName = parameterService.getString(Parameter.FACEBOOK_FOLDER_NAME);
		...
		try {
			...
			codeMessageService.logInfo("DST-FACEBOOK-001", probe.getId(), script);
			LOGGER.error("[DST-FACEBOOK-001] Error starting process", e);
		...
		if (json == null || !json.has("time")) {
			codeMessageService.logInfo("DST-FACEBOOK-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		if (json.optBoolean("error")) {
			if ("DST-FACEBOOK-003".equals(code)) {
		...
		try {
			...
			LOGGER.error("[DST-FACEBOOK-002] Error getting time from json", e);
			codeMessageService.logInfo("DST-FACEBOOK-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		LOGGER.info("Facebook test executed successfully and stored in [{}]", result);
	}
