...
import net.kiwox.manager.dst.appium.service.interfaces.IInstagramTestService;
...
@Service
public class InstagramTestServiceImpl implements IInstagramTestService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(InstagramTestServiceImpl.class);
	...
	public void runTest(ControllerProbe probe, TestResult result) {
		...
		script.append(" -user \"");
		script.append(parameterService.getString(Parameter.INSTAGRAM_USERNAME));
		script.append("\" -pass \"");
		script.append(parameterService.getString(Parameter.INSTAGRAM_PASSWORD));
		script.append("\"");
		
		Test test = result.getTest();
		
		String folderName = parameterService.getString(Parameter.INSTAGRAM_FOLDER_NAME);
		if (StringUtils.isNotEmpty(folderName)) {
			script.append(" -folder \"");
			script.append(folderName);
			script.append("\"");
		}
		...
		try {
			...
			codeMessageService.logInfo("DST-INSTAGRAM-001", probe.getId(), script);
			LOGGER.error("[DST-INSTAGRAM-001] Error starting process", e);
		...
		if (json == null || !json.has("time")) {
			codeMessageService.logInfo("DST-INSTAGRAM-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		if (json.optBoolean("error")) {
			if ("DST-INSTAGRAM-003".equals(code)) {
		...
		try {
			...
			LOGGER.error("[DST-INSTAGRAM-002] Error getting time from json", e);
			codeMessageService.logInfo("DST-INSTAGRAM-002", probe.getId(), script);
			appiumTestService.markFailed(result);
			return;
		}
		...
		LOGGER.info("Instagram test executed successfully and stored in [{}]", result);
	}
